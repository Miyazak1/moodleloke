...
const {
  ...
  buildAuditBatch
} = require('./csca-subject-practice-physics-kinematics-local-audit-export.cjs');

const SCORE_POLICY_VERSION = 'physics-kinematics-local-blind-audit-score-v1';
...
function validateInputs({ manifest, blindReviewPacket, answerKey, reviewResponse }) {
  ...
  if (reviewResponse?.blindPacketSha256 !== manifest?.blindPacketSha256) {
    failureCodes.push('review_response_blind_packet_binding_mismatch');
  }
  const attestation = recordFrom(reviewResponse?.reviewerAttestation);
  if (!attestation || typeof attestation.reviewerId !== 'string' || !attestation.reviewerId.trim()) {
    failureCodes.push('reviewer_identity_missing');
  }
  if (attestation?.reviewedWithoutAnswerKey !== true) failureCodes.push('reviewer_blind_attestation_missing');
  if (typeof attestation?.lockedAt !== 'string' || !Number.isFinite(Date.parse(attestation.lockedAt))) {
    failureCodes.push('review_lock_timestamp_missing_or_invalid');
  }
  ...
  if (questionIds.length !== manifest?.selectedAuditCount || new Set(questionIds).size !== questionIds.length) {
    failureCodes.push('blind_packet_candidate_set_invalid');
  }
  if (answerIds.length !== questionIds.length || new Set(answerIds).size !== answerIds.length) {
    failureCodes.push('answer_key_candidate_set_invalid');
  }
  if (reviewIds.length !== questionIds.length || new Set(reviewIds).size !== reviewIds.length) {
    failureCodes.push('review_response_candidate_set_invalid');
  }
  ...
  for (const question of questions) {
    ...
    const semanticFingerprint = sha256({
      ...
      optionTextsIgnoringPosition: Array.isArray(question?.options)
        ? question.options.map((option) => option?.text).sort()
        : []
    });
    if (question?.semanticFingerprint !== semanticFingerprint) {
      failureCodes.push('blind_packet_semantic_fingerprint_mismatch');
    }
  }
  for (const review of reviews) {
    ...
    if (question && !question.options.some((option) => option?.id === review?.selectedOptionId)) {
      failureCodes.push('review_selected_option_not_present');
    }
    ...
    if (typeof review?.unitConsistency !== 'boolean') failureCodes.push('review_unit_consistency_verdict_invalid');
    if (!['pass', 'minor_issue', 'fail'].includes(review?.languageQuality)) {
      failureCodes.push('review_language_quality_invalid');
    }
...
function metricsFor(rows) {
  ...
  const unitConsistencyCount = count((row) => row.review.unitConsistency);
  const languagePassCount = count((row) => row.review.languageQuality === 'pass');
  const languageMinorIssueCount = count((row) => row.review.languageQuality === 'minor_issue');
  ...
  return {
    ...
    unitConsistencyCount,
    unitConsistencyRate: rate(unitConsistencyCount, total),
    languagePassCount,
...
function belowThreshold(metrics) {
  return (metrics.answerAgreementRate ?? 0) < THRESHOLDS.minimumAnswerAgreementRate
    ...
    || (metrics.unitConsistencyRate ?? 0) < THRESHOLDS.minimumUnitConsistencyRate
  ...
  const rows = validation.reviews.map((review) => {
    ...
    return {
      ...
      strictQualified: answerAgrees
        ...
        && review.unitConsistency
        && review.languageQuality === 'pass'
    };
  });
  const overall = metricsFor(rows);
  const byScope = Object.fromEntries(EXPECTED_SCOPES.map((scopeId) => [
    scopeId,
    metricsFor(rows.filter((row) => row.scopeId === scopeId))
  ]));
  const failureCodes = [];
  if (overall.total < THRESHOLDS.minimumTotal) failureCodes.push('audit_sample_size_insufficient');
  if (EXPECTED_SCOPES.some((scopeId) => byScope[scopeId].total < THRESHOLDS.minimumPerScope)) {
    failureCodes.push('audit_scope_coverage_insufficient');
  }
  if ((overall.answerAgreementRate ?? 0) < THRESHOLDS.minimumAnswerAgreementRate) {
    failureCodes.push('audit_answer_agreement_below_threshold');
  }
  if ((overall.solvableRate ?? 0) < THRESHOLDS.minimumSolvableRate) failureCodes.push('audit_solvable_rate_below_threshold');
  if ((overall.uniqueAnswerRate ?? 0) < THRESHOLDS.minimumUniqueAnswerRate) {
    failureCodes.push('audit_unique_answer_rate_below_threshold');
  }
  if ((overall.syllabusAlignedRate ?? 0) < THRESHOLDS.minimumSyllabusAlignedRate) {
    failureCodes.push('audit_syllabus_alignment_below_threshold');
  }
  if ((overall.unitConsistencyRate ?? 0) < THRESHOLDS.minimumUnitConsistencyRate) {
    failureCodes.push('audit_unit_consistency_below_threshold');
  }
  if ((overall.languagePassRate ?? 0) < THRESHOLDS.minimumLanguagePassRate) {
    failureCodes.push('audit_language_pass_rate_below_threshold');
  }
  if ((overall.strictQualifiedRate ?? 0) < THRESHOLDS.minimumStrictQualifiedRate) {
    failureCodes.push('audit_strict_qualified_rate_below_threshold');
  }
  if (EXPECTED_SCOPES.some((scopeId) => belowThreshold(byScope[scopeId]))) {
    failureCodes.push('audit_scope_quality_below_threshold');
  }
...
function runSelfTest() {
  const batch = buildAuditBatch({ countPerScope: 8, auditPerScope: 8, auditSeed: 'score-self-test' });
  const reviewResponse = completeFixtureReview(batch);
  const passed = scoreAuditBatch({ ...batch, reviewResponse });
  if (!passed.qualityGatePassed || passed.overall.total !== 32) {
    throw new Error('Complete valid audit fixture must pass.');
  }
  ...
  if (wrongAnswer.qualityGatePassed || !wrongAnswer.failureCodes.includes('audit_answer_agreement_below_threshold')) {
    throw new Error('Wrong reviewer answer must fail the zero-error answer threshold.');
  }
  const unitFailureResponse = JSON.parse(JSON.stringify(reviewResponse));
  unitFailureResponse.reviews[0].unitConsistency = false;
  const unitFailure = scoreAuditBatch({ ...batch, reviewResponse: unitFailureResponse });
  if (unitFailure.qualityGatePassed
    || !unitFailure.failureCodes.includes('audit_unit_consistency_below_threshold')
    || !unitFailure.failureCodes.includes('audit_scope_quality_below_threshold')) {
    throw new Error('A unit inconsistency must fail both overall and per-scope gates.');
  }
  const nonBlindResponse = JSON.parse(JSON.stringify(reviewResponse));
  ...
  if (nonBlind.validEvidence || !nonBlind.failureCodes.includes('reviewer_blind_attestation_missing')) {
    throw new Error('Non-blind review attestation must invalidate evidence.');
  }
  ...
  if (tampered.validEvidence || !tampered.failureCodes.includes('blind_packet_hash_mismatch')) {
    throw new Error('Tampered blind packet must invalidate evidence.');
  }
  ...
  if (incomplete.validEvidence || !incomplete.failureCodes.includes('review_response_candidate_set_mismatch')) {
    throw new Error('Incomplete review set must invalidate evidence.');
  }
  return {
    mode: 'physics_kinematics_local_blind_audit_score_self_test',
    status: 'passed',
    scorePolicyVersion: SCORE_POLICY_VERSION,
    positiveFixture: passed.status,
    negativeFixtures: {
      wrongAnswer: wrongAnswer.status,
      unitInconsistency: unitFailure.status,
      nonBlindAttestation: nonBlind.status,
